...
namespace AElf.Network
{
    public class UPnP
    {
        ...
        public static async Task<bool> DiscoverAsync()
        {
            ...
            s.ReceiveTimeout = (int)_timeout.TotalMilliseconds;
        ...
        public static async Task CreateForwardingRuleAsync(int port, ProtocolType protocol, string description)
        {
            ...
            XmlDocument xdoc = await SOAPRequestAsync(_serviceUrl, 
                ...
                "</NewExternalPort><NewProtocol>" + protocol.ToString().ToUpper() + "</NewProtocol>" +
                "<NewInternalPort>" + port.ToString() + "</NewInternalPort><NewInternalClient>" + 
                (await Dns.GetHostAddressesAsync(Dns.GetHostName())).First(p => p.AddressFamily == AddressFamily.InterNetwork).ToString() +
